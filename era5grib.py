...
catalogue = pandas.read_csv('catalogue.csv', index_col=['product','varname'])
...
def land_subset(da):
    ...
    da['longitude'] = numpy.concatenate([da.longitude[:3600//2], da.longitude[3600//2:] + 360])
...
def get_weights():
    source = xarray.open_dataset('/g/data/ub4/era5/netcdf/land/skt/2020/skt_era5land_global_20200101_20200131.nc', chunks=chunks['land'])['skt']
    source = land_subset(source.isel(time=0))
    target = fx.lsm.where(fx.lsm > 0)

    source.to_netcdf('source.nc')
    ...
    for k,v in weights.items():
        encoding[k] = {'zlib': True, 'shuffle': True, 'complevel': 4}
        if v.dtype == 'float64':
...
def read_era5_raw(entry, start, end):
    ...
    for ms,me in zip(pandas.date_range(t0,t1,freq='MS'), pandas.date_range(t0,t1, freq='M')):
        path = glob(os.path.join(entry['dirname'], ms.strftime('%Y'), f'*_{ms.strftime("%Y%m%d")}_{me.strftime("%Y%m%d")}.nc'))
        paths.extend(path)

    try:
        da = xarray.open_mfdataset(paths, chunks=chunks[product])[var]
    except OSError:
        raise IndexError('ERROR: No ERA5 data found, check model dates are within the ERA5 period, you are a member of ub4 and that -lstorage includes gdata/ub4')
    ...
    if product == 'surface':
        ...
        da['longitude'] = numpy.concatenate([da.longitude[:1440//2], da.longitude[1440//2:] + 360])
...
def save_grib(ds, output):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        subprocess.run(['cdo','-f','grb1','-t','ecmwf','copy',tmpnc,output], check=True)
...
def read_um(time):
    ...
    ds = read_era5(
        ['skt','sp','siconc','sde','stl1','stl2','stl3','stl4','swvl1','swvl2','swvl3','swvl4'],
        ['u','v','t','q'],
        start,
        start)
...
def read_wrf(start, end):
    ds = read_era5(
        ['u10','v10','t2m','d2m','sp','msl','skt','siconc','sst','sde',
...
def era5grib_wrf(start=None, end=None, output=None, namelist=None, geo=None):
    if namelist is not None:
        with open(namelist, 'r') as f:
            nml = f90nml.read(f)
            if start is None:
                start = pandas.to_datetime(nml['share']['start_date'], format='%Y-%m-%d_%H:%M:%S').min()
            if end is None:
                start = pandas.to_datetime(nml['share']['end_date'], format='%Y-%m-%d_%H:%M:%S').max()
    ...
    if geo is not None:
        ...
        if (geo.XLAT_C.max() > ds.latitude[0] or geo.XLAT_C.min() < ds.latitude[-1] or
            geo.XLONG_C.min() < ds.longitude[0] or geo.XLONG_C.max() > ds.longitude[-1]):
            raise IndexError('Selected domain is outside the NCI ERA5 archive')

        ds = ds.sel(latitude=slice(geo.XLAT_C.max()+1, geo.XLAT_C.min()-1),
...
def era5grib_um(time, output=None, target=None):
    ...
    if target is not None:
        ...
        if (lat.max() > ds.latitude[0] or lat.min() < ds.latitude[-1] or
            lon.min() < ds.longitude[0] or lon.max() > ds.longitude[-1]):
            raise IndexError('Selected domain is outside the NCI ERA5 archive')

        ds = ds.sel(latitude=slice(lat.max()+1, lat.min()-1),
...
def main():
    ...
    um.add_argument('--target', help='UM file on the target grid for trimming (e.g. qrparm.mask)')

    args = parser.parse_args()

    weights = xarray.open_dataset('regrid_weights.nc')
    global regridder
