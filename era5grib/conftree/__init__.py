...
class ConfTree:
    # Constructors
    def __init__(
        self, name: str = "root", data=NoData, children=NoChildren, none_is_default=True
    ):
        self.name = name
    ...
    def from_dict(cls, d, name="root", none_is_default=True):
        ...
        for k, v in d.items():
            if isinstance(v, dict):
                out.children[k] = ConfTree.from_dict(
                    v, k, none_is_default=none_is_default
                )
            else:
            ...
            if len(k_arr) == 1:
                if isinstance(value, dict):
                    if self.children[k_arr[0]].data is not None:
                        raise Exception(
                            "Attempted to update children on ConfTree with data"
                        )
