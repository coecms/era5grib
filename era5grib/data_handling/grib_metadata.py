    ...
    def __init__(self):
        ...
        if c is None:
            if conf.get("format") == "grib":
                log.warn(
                    "WARNING: Unable to find ECMWF metadata catalogue and GRIB format selected. GRIB field metadata will "
                    "NOT correspond to input field metadata"
                )
                self.params = None
                return
