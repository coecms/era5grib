...
class InterpolatingRegridder:
    ...
    def __call__(self, field):
        ...
        for lat in (0, -1):
            for lon in (0, -1):
                out = xr.where(
                    (out.latitude == out.latitude[lat])
                    & (out.longitude == out.longitude[lon])
                    & out.isnull(),
                    fmean,
                    out,
                )
        lat_first = out.interpolate_na(
            dim="latitude", method="bilinear", use_coordinate=False
        ).interpolate_na(dim="longitude", method="bilinear", use_coordinate=False)
        lon_first = out.interpolate_na(
            dim="longitude", method="bilinear", use_coordinate=False
        ).interpolate_na(dim="latitude", method="bilinear", use_coordinate=False)
    ...
    for ts in conf.get_month_range()[1:]:
        for (field_name, ds), field in fields[ts].items():
            ...
            if key not in fields_to_merge:
                die(
                    f"Error: {key} in timestamp {ts} of catalogue search results"
                    f", but not in first timestep {conf.get_month_range()[0]}"
                )
            for realm, da in field.get_dataarrays():
                if da.attrs["source"] in custom_fields:
                    continue
                ...
                if realm not in fields_to_merge[key]:
                    die(
                        f"Error: {key} on {realm} in timestamp {ts} of catalogue"
                        f" search results but not in first timestep {conf.get_month_range()[0]}"
                    )
            ...
            if not da.time.equals(time_dim):
                ...
                if da.attrs["source"] in custom_fields:
                    if len(time_dim) != len(da.time):
                        die(
                            f"Error! Custom field {field_name} has different time"
                            f"dimension length than remaining data: {len(time_dim)}, {len(da.time)}"
                        )
    ...
    if regrid in example_das:
        ...
        if ref_field is None or ref_date is None:
            die("Regridding to unloaded dataarray requested, but regridding parameters "
                "'ref_field' and 'ref_date' are not set")
        target_da = get_single_field(ref_field, regrid, Timestamp(ref_date))
    ...
    for source, da in example_das.items():
        if da.latitude.equals(target_da.latitude) and da.longitude.equals(target_da.longitude):
            ...
            if regrid_options == "weight_file":
                ...
                regridders[source] = xesmf.Regridder(
                    da.to_dataset(),
...
def soil_level_metadata(ds):
    ...
    for lev in range(1, 5):
        ds[f"stl{lev}_surf"] = ds[f"stl{lev}_surf"].expand_dims(f"depth{lev}", axis=1)
        ds[f"swvl{lev}_surf"] = ds[f"swvl{lev}_surf"].expand_dims(f"depth{lev}", axis=1)
        ds.coords[f"depth{lev}"] = xr.DataArray(depth[lev: (lev + 1)], dims=[f"depth{lev}"], attrs=depth_attrs)
        ds.coords[f"depth{lev}_bnds"] = xr.DataArray([depth_bnds[lev: (lev + 2)]], dims=[f"depth{lev}", "bnds"], attrs=depth_attrs)  # noqa: E501
        ds.coords[f"depth{lev}"].attrs["bounds"] = f"depth{lev}_bnds"
